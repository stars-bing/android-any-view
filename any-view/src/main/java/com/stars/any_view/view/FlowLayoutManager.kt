    ...
    private fun fill(recycler: RecyclerView.Recycler, state: RecyclerView.State, dy: Int): Int {
        ...
        if (dy >= 0) {
            ...
            if (childCount > 0) {
                ...
                lineMaxHeight =
                    max(lineMaxHeight.toDouble(), getDecoratedMeasurementVertical(lastView).toDouble()).toInt()
            }
            //顺序addChildView
            for (i in minPos..mLastVisiPos) {
                ...
                if (leftOffset + getDecoratedMeasurementHorizontal(child) <= getHorizontalSpace()) { //当前行还排列的下
                    ...
                    leftOffset += getDecoratedMeasurementHorizontal(child)
                    lineMaxHeight =
                        max(lineMaxHeight.toDouble(), getDecoratedMeasurementVertical(child).toDouble()).toInt()
                } else { //当前行排列不下
                    ...
                    if (topOffset - dy > height - paddingBottom) {
                        ...
                        //改变 left  lineHeight
                        leftOffset += getDecoratedMeasurementHorizontal(child)
                        lineMaxHeight = max(lineMaxHeight.toDouble(), getDecoratedMeasurementVertical(child).toDouble())
                            .toInt()
                    }
                }
            }
